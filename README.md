...
All real market data, fetched through the `/api/prices.js` serverless proxy (no API key). Chain per ticker:

1. **Yahoo Finance** chart API (the endpoint `yfinance` uses) — with a cookie+crumb session like yfinance establishes, tried on both `query1` and `query2` hosts. Adjusted closes.
2. **Stooq** EOD CSV, server-side — fallback when Yahoo rate-limits datacenter IPs (HTTP 429), which is common on shared cloud egress. Split-adjusted closes.
3. If both fail → ticker is marked **No data**, shown blank, excluded from composites and pairs.

**There is no mock data.** Both sources are real exchanges' EOD prices; the scan status reports the mix (e.g. `50/50 loaded (42 Yahoo · 8 Stooq)`). Responses are edge-cached for 1 hour, so a re-run scan is instant and doesn't re-hit the providers.
