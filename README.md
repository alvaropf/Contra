...
2. Click **Run Scan** — pulls ~3 years of daily closes for the 146-ETF default universe (Alvaro's ETF_PreselectedList: bonds, China, commodities, crypto, DM, EM, India, tech, US) from Yahoo Finance; tickers that fail to load show as "No data" and are excluded
...
## Data source — Python + yfinance (Beta_Pf / Intermarket pattern)

`api/snapshot.py` is a Vercel **Python** serverless function that pulls the whole universe with **yfinance** — the same architecture as the Intermarket Action Sheet's `daily-snapshot.py`, which is proven to work on Vercel. Why this works where raw HTTP requests got 429'd: yfinance handles Yahoo's session/cookie dance internally, and the frontend makes **one batched request for all ~50 tickers** instead of 50 separate hits, so Yahoo never sees a request storm.

- `GET /api/snapshot?tickers=SPY,QQQ,...` → `{ "SPY": [{"d":"YYYY-MM-DD","c":123.45}, ...], ... }`
- Batched `yf.download` in chunks of 25 (`threads=True`), `auto_adjust=True` (dividend/split-adjusted closes), ~4y history capped at 820 bars
- In-memory cache 6h on warm instances + edge cache (`s-maxage=21600`), so after the first scan of the day every subsequent one is instant
- Bounded forward-fill (5-day limit) bridges real gaps without fabricating history for padded/delisted tickers
- A ticker yfinance can't deliver is simply absent → shown as **No data**, excluded from composites and pairs. **There is no mock data.**
- Cold start fetches everything and can take ~20s; Vercel Hobby's 60s function limit covers it comfortably at this universe size
...
```bash
npm i -g vercel
vercel dev   # runs the Python function locally too (installs requirements.txt)
```
...
C1/C2/C3 form a chain, not a checklist: **C2 only counts if C1 holds; C3 only counts if C1 and C2 hold.** Scores: 0 = no capitulation; 1 = crashed, still making lows; 2 = crashed and lows exhausted; 3 = full Stage 1 setup. A tight range without a prior crash is consolidation, not a base — it scores 0. The detail view marks conditions whose raw threshold is met but whose chain is broken as "gated".
